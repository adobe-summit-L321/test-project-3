...
import * as initSearchPopover from './searchbar.js';
...
function createClearedSelectionsButton() {
  ...
  closeBtnDiv.style.display = window.localStorage.getItem('selectedBrand') && window.localStorage.getItem('selectedModel') ? 'block' : 'none';
...
export default async function decorate(block) {
  ...
  if (navSections) {
    ...
    const selectedBrand = brandIds[window.localStorage.getItem('selectedBrand')] || '';
    const selectedModel = (selectedBrand && window.localStorage.getItem('selectedModel')) || '';
    ...
    const dropDownClickHandler = (navItem) => () => {
      ...
      renderPlpDropin({ ...(ddModel ? { 'AC-Policy-Model': ddModel } : {}) });
    };
  ...
  async function toggleSearch(state) {
    ...
    if (show) {
      ...
      const modelHeaders = selectedModel ? { 'AC-Policy-Model': selectedModel } : {};
      initSearchPopover(modelHeaders);
      searchInput.focus();
    }
  }
